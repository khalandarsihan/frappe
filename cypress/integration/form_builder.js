...
context("Form Builder", () => {
	...
	it("Save without change, check form dirty", () => {
		...
		cy.get(".desk-alert.orange .alert-message").should(
			"have.text",
		...
		cy.get(
			...
			.dblclick()
			.type("Dirty");
		cy.get(".title-area .indicator-pill.orange").should(
			"have.text",
			"Not Saved",
		);
	});
	...
	it("Add empty section and save", () => {
		...
		// save
		cy.click_doc_primary_button("Save");
		cy.get(".tab-content.active .form-section-container").should(
			"have.length",
			1,
		);
	});
	...
	it("Add Table field and check if columns are rendered", () => {
		...
		let first_column =
			".tab-content.active .section-columns-container:first .column:first";
		...
		cy.get(".combo-box-options:visible .search-box > input").type(
			"table{enter}",
		);
		...
		cy.get_open_dialog()
			.find(".msgprint")
			.should("contain", "Options is required");
		cy.hide_dialog();

		cy.get(last_field).click({ force: true });

		cy.get(
			".sidebar-container .frappe-control[data-fieldname='options'] input",
		)
			.click()
			.as("input");
		cy.get("@input")
			.clear({ force: true })
			.type("Web Form Field", { delay: 200 });
		cy.wait("@search_link");
		...
		cy.get(".sidebar-container .field label .label-area")
			.contains("In List View")
			.click();
		...
		cy.get_open_dialog()
			.find(".msgprint")
			.should("contain", "In Global Search");
	});
	...
	it.skip("Drag Field/Column/Section & Tab", () => {
		...
		cy.get(".tab-header .tabs .tab:first").drag(
			".tab-header .tabs .tab:nth-child(2)",
			{
				target: { x: 10, y: 10 },
				force: true,
			},
		);
		cy.get(".tab-header .tabs .tab:first")
			.find(label)
	...
	it("Update Title field Label to New Title through Customize Form", () => {
		...
		cy.get("[data-fieldname='data3'] .clearfix label").should(
			"have.text",
			"New Title",
		);
	});
	...
	it("Validate Duplicate Name & reqd + hidden without default logic", () => {
		...
		cy.get(
			".sidebar-container .frappe-control[data-fieldname='fieldname'] input",
		)
			.click()
			.as("input");
		cy.get(
		...
		cy.get_open_dialog()
			.find(".msgprint")
			.should("contain", "appears multiple times");
		cy.hide_dialog();
		cy.get(last_field).click();
		cy.get(
			".sidebar-container .frappe-control[data-fieldname='fieldname'] input",
