context("Data Control", () => {
	...
	it('Verifying data control by inputting different patterns for "Name" field', () => {
		...
		cy.get('.frappe-control[data-fieldname="name1"]').should(
			"have.class",
			"has-error",
		);
		cy.save();
	...
	it('Verifying data control by inputting different patterns for "Email" field', () => {
		...
		cy.get('.frappe-control[data-fieldname="email"]').should(
			"have.class",
			"has-error",
		);
		cy.save();
		...
		cy.get(".msgprint").should(
			"have.text",
			"komal is not a valid Email Address",
		);
		cy.hide_dialog();
		cy.get_field("email", "Data").clear({ force: true });
		...
		cy.get(".msgprint").should(
			"have.text",
			"komal@test is not a valid Email Address",
		);
		cy.hide_dialog();
	});
	...
	it('Verifying data control by inputting different patterns for "Phone" field', () => {
		...
		cy.get('.frappe-control[data-fieldname="phone"]').should(
			"have.class",
			"has-error",
		);
		...
		cy.get(".msgprint").should(
			"have.text",
			"komal is not a valid Phone Number",
		);
		cy.hide_dialog();
	});
	...
	it("Deleting the doc", () => {
		...
		cy.get(
			'.actions-btn-group > .dropdown-menu [data-label="Delete"]',
		).click();
