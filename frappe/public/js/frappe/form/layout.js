...
frappe.ui.form.Layout = class Layout {
	...
	show_message(html, color) {
		...
		let close_message = $(
			`<div class="close-message">${frappe.utils.icon("close")}</div>`,
		);
		this.message_color =
			color &&
			["yellow", "blue", "red", "green", "orange"].includes(color)
				? color
				: "blue";
		if (html) {
			if (html.substr(0, 1) !== "<") {
	...
	select_tab(label_or_fieldname) {
		for (let tab of this.tabs) {
			if (
				...
				tab.df.fieldname?.toLowerCase() ===
					label_or_fieldname.toLowerCase()
			) {
				tab.set_active();
				return;
			}
