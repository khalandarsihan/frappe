...
frappe.ui.form.save = function (frm, action, callback, btn) {
	...
	const working_label = {
		...
		Submit: __(
			"Submitting",
			null,
			"Freeze message while submitting a document",
		),
		Update: __(
			"Updating",
			null,
			"Freeze message while updating a document",
		),
		Amend: __("Amending", null, "Freeze message while amending a document"),
		Cancel: __(
			"Cancelling",
			null,
			"Freeze message while cancelling a document",
		),
	}[toTitle(action)];
				...
				function (i, docfield) {
					if (docfield.fieldname) {
						...
						if (
							...
							has_errors = true;
							error_fields[error_fields.length] = __(
								df.label,
								null,
								df.parent,
							);
			...
			if (error_fields.length) {
				...
				if (meta.istable) {
					...
					const table_label = __(
						table_field.label ||
							frappe.unscrub(table_field.fieldname),
					).bold();
					...
					message = __("Mandatory fields required in {0}", [
						__(doc.doctype),
					]);
				}
				message =
