...
export default class OnboardingWidget extends Widget {
	...
	make_body() {
		...
		let first_incomplete_step = this.steps.findIndex(
			(s) => !s.is_skipped && !s.is_complete,
		);
			...
			() => {
				if (frappe.get_route_str() != current_route) {
					...
					frappe.show_alert(
						__("Document Saved") +
							"<br>" +
							__("Let us continue with the onboarding"),
					);
					this.mark_complete(step);
				}
			},
		...
		return frappe.model
			...
			.then((onboarding_doc) => {
				if (onboarding_doc) {
					...
					this.title =
						onboarding_doc.title || __("Let's Get Started");
	...
	async get_first_document(doctype) {
		...
		if (message.first_document) {
			await frappe.db
				.get_list(doctype, { order_by: "creation" })
				.then((res) => {
