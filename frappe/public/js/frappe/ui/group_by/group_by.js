...
frappe.ui.GroupBy = class {
	...
	apply_settings(settings) {
		...
		if (
			...
			const aggregate_on_doctype =
				this.get_aggregate_on_doctype(settings);
			settings.aggregate_on =
				"`tab" +
	...
	get_group_by_docfield() {
		...
		if (this.aggregate_function === "count") {
			...
			if (this.aggregate_function === "sum") {
				...
				docfield.label = __("Average of {0}", [
					__(docfield.label, null, docfield.parent),
				]);
			}
		}
	...
	get_group_by_fields() {
		...
		table_fields.forEach((df) => {
			...
			const child_table_fields = frappe.meta
				...
				.sort((a, b) =>
					__(cstr(a.label)).localeCompare(__(cstr(b.label))),
				);
