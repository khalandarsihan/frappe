...
frappe.views.ReportView = class ReportView extends frappe.views.ListView {
	...
	get_editing_object(colIndex, rowIndex, value, parent) {
		...
		return {
			...
			setValue: (value) => {
				...
				let fieldname =
					this.datatable.getColumn(colIndex).docfield.fieldname;
	...
	is_column_added(df) {
		return Boolean(
			this.fields.find(
				(f) => f[0] === df.fieldname && df.parent === f[1],
			),
		);
	}
		...
		let items = [
			...
			},
			{
				...
				action: () => {
					...
					const rows_in_order =
						this.datatable.datamanager.rowViewOrder
							.map((index) => {
								if (
			...
			items.push({
				...
				action: () => {
					...
					if (this.list_view_settings.disable_count) {
						...
						this.total_count >
						(this.count_without_children || args.page_length)
					) {
						extra_fields = [
							{
								fieldtype: "Check",
